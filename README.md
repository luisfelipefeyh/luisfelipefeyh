<h1 align="center">Hi!👋 I'm Luis.</h1>
[![Anurag's GitHub stats](https://github-readme-stats.vercel.app/api?username=luisfelipefeyh)](https://github.com/luisfelipefeyh/github-readme-stats)
<p align="center">
  Web Developer • Florianópolis, Brazil • Information Systems @ UFSC
</p>

---

## 🇧🇷 Sobre mim

- 🎓 Estudante de **Sistemas de Informação** na **UFSC**
- 💼 Estagiário em desenvolvimento, trabalhando com **aplicações web** e **players embarcados**
- 💻 Interesse em **backend**, **web**, **bancos de dados** e **engenharia de software**

### 🔧 Tecnologias

- **Linguagens:** JavaScript, TypeScript, Python  
- **Web:** NestJS, Angular, HTML, CSS  
- **Bancos de dados:** PostgreSQL, MySQL  
- **Ferramentas:** Git, Linux

### 📫 Contato

- Email: `luisfelipe@feyh.com.br`  
- LinkedIn: [www.linkedin.com/in/luisfelipefeyh](https://www.linkedin.com)

---

## 🇺🇸 About me

- 🎓 **Information Systems** student at **UFSC** (Florianópolis, Brazil)
- 💼 Trainee developer working with **web apps** and **embedded players**
- 💻 Interested in **backend**, **web**, **databases** and **software engineering**

### 🔧 Tech

- **Languages:** JavaScript, TypeScript, Python  
- **Web:** NestJS, Angular, HTML, CSS  
- **Databases:** PostgreSQL, MySQL  
- **Tools:** Git, Linux

### 📫 Contact

- Email: `luisfelipe@feyh.com.br`  
- LinkedIn: [www.linkedin.com/in/luisfelipefeyh](https://www.linkedin.com)
